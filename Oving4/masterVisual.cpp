...
void addGuess(MastermindWindow &mwin, const string code, const int size, const string guess, const int round, map<char, Color> colors)
{
	int xPos;
    int yPos = 2 * padY + round * 2 * padY;
    
    for(unsigned int i = 0; i < size; i++){
        xPos = (2*i + 1) * padX;
        mwin.vr.push_back(new Rectangle{Point{xPos,yPos},padX,padY});
        mwin.vr[mwin.vr.size()-1].set_fill_color(colors.at(guess[i]));
				 ...
				 const int round)
{
    ...
    for(int i = 0; i < size; i++){
        ...
        mwin.vc[mwin.vr.size()-1].set_fill_color(Color::black);
        mwin.attach(mwin.vc[mwin.vr.size()-1]);
    }
}
